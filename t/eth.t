...
my @received_transactions = $received_transactions->@*;
...
note explain $received_transactions;
is $received_transactions, undef, "invalid transaction filtered";

$receipt->{result}->{status} = "0x1";

$received_transactions = $subscription_client->_check_contract_transaction($transaction, $receipt->{result})->get;
is $received_transactions, undef, "invalid transaction filtered even when the status is true";
