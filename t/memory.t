...
my $decoded_transaction = decode_json_utf8(
...
my $decoded_contract_transaction = decode_json_utf8(
...
$module_rpc->mock(
    'get_transaction_receipt' => async sub {
        ...
        if ($tx eq "0x1a7d89fcbba627f9c82ac8edcf93180c84a5ae754418589787a703ad4a974870") {
            ...
            return $decoded_contract_transaction;
        }
    },
    'accounts' => async sub {
        return $accounts;
    ...
    'call' => async sub {
        my (undef, $data_param) = @_;
        if ($data_param->{data} eq Net::Async::Blockchain::ETH::SYMBOL_SIGNATURE) {
            return '0x0000000000000000000000000000000000000000000000000000000045544844';
        } elsif ($data_param->{data} eq Net::Async::Blockchain::ETH::DECIMALS_SIGNATURE) {
...
subtest "Ethereum memory test" => sub {
    my $invalid_transaction = decode_json_utf8(
    ...
    no_growth {
        $eth_client->transform_transaction($invalid_transaction, time);
    }
    'Subscription for ETH/ERC20 no memory growth for invalid transaction';

    my $contract_transaction = decode_json_utf8(
    ...
    no_growth {
        $eth_client->transform_transaction($contract_transaction, time);
    }
